...
@RestController
@RequestMapping("/lifereply")
public class LifeReplyController {
	@Setter(onMethod_ = @Autowired)
	private LifeReplyService service;
	
	@GetMapping("/board/{boardId}")
	public List<LifeReplyVO> list(@PathVariable Integer boardId, HttpSession session) {
		UserVO loggedIn = (UserVO) session.getAttribute("loginUser");
		List<LifeReplyVO> list = service.list(boardId);
		if(loggedIn != null) {
			for (LifeReplyVO reply : list) {
	...
	@PostMapping("/write")
	public ResponseEntity<String> write(LifeReplyVO reply, @SessionAttribute(value="loginUser", required = false) UserVO logged, HttpSession session) {
//		logged = (UserVO) session.getAttribute("loginUser");
	...
	@PutMapping("/{id}")
	public ResponseEntity<String>  modify(@PathVariable Integer id, @RequestBody LifeReplyVO  reply, @SessionAttribute(value="loginuser", required = false) UserVO logged, HttpSession session) {
		// 댓글 조회
		logged = (UserVO) session.getAttribute("loginUser");
		
		LifeReplyVO old = service.readById(id);
