...
public class QuestionReplyController {
	...
	public List<QuestionReplyVO> list(@PathVariable Integer boardId, HttpSession session) {
		UserVO loggedIn = (UserVO) session.getAttribute("loginUser");
		List<QuestionReplyVO> list = service.list(boardId);
		if(loggedIn != null) {
			for (QuestionReplyVO reply : list) {
...
//		logged = (UserVO) session.getAttribute("loginUser");
	...
	public ResponseEntity<String>  modify(@PathVariable Integer id, @RequestBody QuestionReplyVO  reply, @SessionAttribute(value="loginuser", required = false) UserVO logged, HttpSession session) {
		...
		QuestionReplyVO old = service.readById(id);
