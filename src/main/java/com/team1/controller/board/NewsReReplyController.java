...
import com.team1.service.board.NewsReReplyService;

import lombok.Setter;

@RestController
@RequestMapping("/newsrereply")
public class NewsReReplyController {
	
	@Setter(onMethod_ = @Autowired)
	private NewsReReplyService service;

	@GetMapping("/reply/{replyId}")
	public List<NewsReReplyVO> list(@PathVariable Integer replyId, HttpSession session) {
		UserVO loggedIn = (UserVO) session.getAttribute("loginUser");
		List<NewsReReplyVO> relist = service.list(replyId); //댓글에 달린 대댓글 리스트 
		if(loggedIn != null) {
			for (NewsReReplyVO rereply : relist) {
	...
	@PostMapping("/write")
	public ResponseEntity<String> write(NewsReReplyVO rereply, @SessionAttribute(value="loginUser", required = false) UserVO logged, HttpSession session) {
	...
	@PutMapping("/{id}")
	public ResponseEntity<String>  modify(@PathVariable Integer id, @RequestBody NewsReReplyVO  rereply, @SessionAttribute(value="loginuser", required = false) UserVO logged, HttpSession session) {
		// 댓글 조회
		logged = (UserVO) session.getAttribute("loginUser");
		
		NewsReReplyVO old = service.readById(id);
