...
<body>
				...
				<div id="buttonmenu" class="row d-flex justify-content-between">
					<div class="col-md-auto my-auto px-auto ">
						<c:if test="${sessionScope.loginUser.id eq post.memberId || sessionScope.loginUser.adminQuali eq 1 }">
							<a href="${pageContext.request.contextPath }/question/modify?id=${post.id }" class="btn btn-outline-secondary 게시글-수정-삭제"> 
							<i class="fas fa-eraser"></i> 수정 / 삭제 </a>
						</c:if>
					</div>

					<div class="col-md-auto my-auto px-auto">
						<a href="${pageContext.request.contextPath }/question/list" class="btn btn-outline-secondary 게시글-목록가기">
					...
					<c:choose>
						<c:when test="${post.reportCount<5 }">
							<form method="post" id="reportForm">
								<input type="hidden" class="form-control" id="reportinput1" name="boardName" value="동네신문고">
			...
			<div class="modal-content">
				...
				<div class="replyReportModalBody modal-body">
					<form method="post" id="replyReportForm">
						<input type="hidden" class="form-control" id="replyreportinput1" name="boardName" value="동네신문고">
						<input type="hidden" class="form-control" id="replyreportinput4" name="categories" value="댓글">
						<input type="hidden" class="form-control" id="replyreportinput5" name="questionReplyId" value="">
						<div class="form-group">
							<label for="reportinput2">작성자</label>
			...
			<div class="modal-content">
				...
				<div class="rereplyReportModalBody modal-body">
					<form method="post" id="rereplyReportForm">
						<input type="hidden" class="form-control" id="rereplyreportinput1" name="boardName" value="동네신문고">
						<input type="hidden" class="form-control" id="rereplyreportinput4" name="categories" value="댓글">
						<input type="hidden" class="form-control" id="rereplyreportinput6" name="questionReplyId" value="">
						<input type="hidden" class="form-control" id="rereplyreportinput5" name="questionReReplyId" value="">
						<div class="form-group">
							<label for="reportinput2">작성자</label>
	...
	$(document).ready(function(){
		...
		$("#reportSubmitButton").click(function(e) {
			...
			const questionId = '${post.id}'; // 현재 게시판의 id, 동네 소식 게시판이 된다면 newsId가 됨.
			const report = {
				boardName : boardName,
				boardUrl : boardUrl,
		...
		$("#replyReportSubmitButton").click(function(e) {
			...
			const questionId = '${post.id}'; // 현재 게시판의 id, 동네 소식 게시판이 된다면 newsId가 됨.
			const questionReplyId = $("#replyreportinput5").val();//댓글 id
			const report = {
				boardName : boardName,
				boardUrl : boardUrl,
				categories : categories,
		...
		$("#rereplyReportSubmitButton").click(function(e) {
			...
			const questionId = '${post.id}'; // 현재 게시판의 id, 동네 소식 게시판이 된다면 newsId가 됨.
			const questionReplyId = $("#rereplyreportinput6").val();//댓글 id
			const questionReReplyId = $("#rereplyreportinput5").val();//대댓글 id
			const report = {
				boardName : boardName,
				boardUrl : boardUrl,
				categories : categories,
			...
			$.ajax({
				...
				const upPost = function() {
					$("#upins").click(function() {
						console.log("업버튼 클릭됨.");
						$.ajax({
			        	url : appRoot + "/questionup/${post.id}",
						type : "put",
	        	...
	        	let a = 0;
	          	for (let i = 0; i < list.length; i++) {
				    ...
				    // 대댓글 리스트 함수 
				      ...
				      $.ajax({
				        url : appRoot + "/questionrereply/reply/" + replyId,
				          	...
				          	for (let i = 0; i < relist.length; i++) {
								...
								rereplyMediaObject.find("#sendReReplyMd" + relist[i].id).click(function() {
									...
									if(replyText.replace(/\s|　/gi, "").length==0){
										...
										//대댓글 수정 요청 
										$.ajax({
											url : appRoot + "/questionrereply/" + relist[i].id,
											type : "put",
								...
								$("#rereplyreport"+relist[i].id).click(function(e) {
									...
									const questionReReplyId = relist[i].id;// 대댓글 id
									const questionReplyId = replyId;
									console.log("questionid : " +questionReplyId);
									$.ajax({
										url : appRoot +"/reportajax/questionrrcount/"+questionReReplyId,
										type : "get",
										success : function(rrcnt){
											if(rrcnt<5){
												...
												$("#rereplyreportinput5").attr("value", questionReReplyId);
												$("#rereplyreportinput6").attr("value", questionReplyId);
					...
					$("#sendReReply"+list[i].id).click(function() {
						...
						if(replyText.replace(/\s|　/gi, "").length==0){
							...
							$.ajax({
								url : appRoot+ "/questionrereply/write",
								type : "post",
								data : data,
								success : function() {
									// textarea reset
					...
					$("#replyreport"+replyId).click(function(e) {
						...
						const questionReplyId = list[i].id // 댓글id
						
						$.ajax({
							url : appRoot +"/reportajax/questionrcount/"+questionReplyId,
							type : "get",
							success : function(rcnt){
								if(rcnt<5){
									...
									$("#replyreportinput5").attr("value", questionReplyId);
	...
	/* 				//각 댓글에 달린 대댓글 개수 세기 
					const listReReplyCount = function() {
						$.ajax({
							url : appRoot+ "/questionrereply/count/"+replyId,
							type : "get",
	...
	/* 	//게시판에 달린 대댓글 개수 세기 (댓글수와 합칠 용도)
		const countInBoard = function() {
			$.ajax({
				url : appRoot+ "/questionrereply/countb/"+${post.id},
				type : "get",
