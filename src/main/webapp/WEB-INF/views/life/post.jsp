...
<body>
				...
				<div id="buttonmenu" class="row d-flex justify-content-between">
					<div class="col-md-auto my-auto px-auto ">
						<c:if test="${sessionScope.loginUser.id eq post.memberId || sessionScope.loginUser.adminQuali eq 1 }">
							<a href="${pageContext.request.contextPath }/life/modify?id=${post.id }" class="btn btn-outline-secondary 게시글-수정-삭제"> 
							<i class="fas fa-eraser"></i> 수정 / 삭제 </a>
						</c:if>
					</div>

					<div class="col-md-auto my-auto px-auto">
						<a href="${pageContext.request.contextPath }/life/list" class="btn btn-outline-secondary 게시글-목록가기">
	...
	$(document).ready(function(){
		...
		$("#reportSubmitButton").click(function(e) {
			...
			const lifeId = '${post.id}'; // 현재 게시판의 id, 동네 소식 게시판이 된다면 newsId가 됨.
			const report = {
				boardName : boardName,
				boardUrl : boardUrl,
		...
		$("#replyReportSubmitButton").click(function(e) {
			...
			const lifeId = '${post.id}'; // 현재 게시판의 id, 동네 소식 게시판이 된다면 newsId가 됨.
			const lifeReplyId = $("#replyreportinput5").val();//댓글 id
			const report = {
				boardName : boardName,
				boardUrl : boardUrl,
				categories : categories,
		...
		$("#rereplyReportSubmitButton").click(function(e) {
			...
			const lifeId = '${post.id}'; // 현재 게시판의 id, 동네 소식 게시판이 된다면 newsId가 됨.
			const lifeReplyId = $("#rereplyreportinput6").val();//댓글 id
			const lifeReReplyId = $("#rereplyreportinput5").val();//대댓글 id
			const report = {
				boardName : boardName,
				boardUrl : boardUrl,
				categories : categories,
	...
	<script>
	  $(document).ready(function() {
	    ...
	    const listReply = function() {
	      ...
	      $.ajax({
	        url : appRoot + "/lifereply/board/${post.id}",
	        	...
	        	let a = 0;
	          	for (let i = 0; i < list.length; i++) {
				    ...
				    // 대댓글 리스트 함수 
				      ...
				      $.ajax({
				        url : appRoot + "/liferereply/reply/" + replyId,
				          	...
				          	for (let i = 0; i < relist.length; i++) {
								...
								rereplyMediaObject.find("#sendReReplyMd" + relist[i].id).click(function() {
									...
									if(replyText.replace(/\s|　/gi, "").length==0){
										...
										//대댓글 수정 요청 
										$.ajax({
											url : appRoot + "/liferereply/" + relist[i].id,
											type : "put",
								...
								$("#rereplyreport"+relist[i].id).click(function(e) {
									...
									const lifeReReplyId = relist[i].id;// 대댓글 id
									const lifeReplyId = replyId;
									console.log("lifeid : " +lifeReplyId);
									$.ajax({
										url : appRoot +"/reportajax/liferrcount/"+lifeReReplyId,
										type : "get",
										success : function(rrcnt){
											if(rrcnt<5){
												...
												$("#rereplyreportinput5").attr("value", lifeReReplyId);
												$("#rereplyreportinput6").attr("value", lifeReplyId);
					...
					replyMediaObject.find("#sendReply" + list[i].id).click(function() {
						...
						if(replyText.replace(/\s|　/gi, "").length==0){
							...
							//댓글 수정 요청 
							$.ajax({
								url : appRoot + "/lifereply/" + list[i].id,
								type : "put",
								contentType : "application/json",
								data : JSON.stringify(data),
								complete : function() {
					...
					$("#replyreport"+replyId).click(function(e) {
						...
						const lifeReplyId = list[i].id // 댓글id
						
						$.ajax({
							url : appRoot +"/reportajax/lifercount/"+lifeReplyId,
							type : "get",
							success : function(rcnt){
								if(rcnt<5){
									...
									$("#replyreportinput5").attr("value", lifeReplyId);
	...
	/* 				//각 댓글에 달린 대댓글 개수 세기 
					const listReReplyCount = function() {
						$.ajax({
							url : appRoot+ "/liferereply/count/"+replyId,
							type : "get",
	...
	/* 	//게시판에 달린 대댓글 개수 세기 (댓글수와 합칠 용도)
		const countInBoard = function() {
			$.ajax({
				url : appRoot+ "/liferereply/countb/"+${post.id},
				type : "get",
		...
		const listReplyCount = function() {
			...
			$.ajax({
				url : appRoot+ "/lifereply/count/"+${post.id},
				type : "get",
