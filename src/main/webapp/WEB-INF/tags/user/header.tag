	...
	<div class="container">
		<div
			...
			<div class="text-end dropdown">
				<!-- 로그인한 상태인 경우 프로필을 띄운다 -->
				<c:if test="${empty sessionScope.loggedInMember }">
                    <button type="button" class="btn btn-info btn-outline-light me-2">
                        Login
                    </button>
				...
				<!-- 로그인 하지 않은 상태의 경우 로그인 버튼과 회원가입 버튼을 띄운다. -->
				<c:if test="${not empty sessionScope.loggedInMember }">
					...
					<span class="align-middle h3 dropdown-toggle"
						...
						aria-expanded="false"> EasyMoneySniper </span>
